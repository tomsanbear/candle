...
|---|---|---|---|
| grid_sample / MS deformable attention kernels | metal+cuda+cpu kernels | markers handoff 2026-07-22 (P2): entire multiscale deformable attention runs as a host-f32 pure-Rust bilinear sampler (`heron_deformable.rs`) — "the largest performance footgun" for layout on GPU | bilinear `grid_sample` op (all backends, F16/F32) as the primitive; `ms_deform_attn` composed on top or as a dedicated kernel. Planned as markers phase-C |
| Device top-k for wide rows | metal-kernels | markers handoff 2026-07-22 (P1): Heron does device max → host F32 → sort. VERIFIED real: the bitonic `asort` is one-thread-per-padded-column with a `ncols_pad`-sized threadgroup array dispatched as `ncols_pad` threads — hard ~1024-column ceiling, so RT-DETR-class rows (num_queries × classes ≈ 24k) cannot run on-device | tile-and-merge top-k kernel (k ≤ 1024, arbitrary ncols): per-row threadgroup walks 1024-wide tiles, bitonic-sorts each, merges into a running sorted top-k in shared memory. Planned as markers phase-B. NOT a dtype gap: `asort_{asc,desc}_f16` already exists |
