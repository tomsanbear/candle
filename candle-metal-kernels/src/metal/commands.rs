...
pub struct Commands {
    ...
    /// completion hook can attribute timings per-kernel.
    compute_per_buffer: AtomicUsize,
    /// Optional global hook installed on every command buffer before commit.
    /// `None` means no instrumentation (zero overhead). Set via
...
impl Commands {
    ...
    /// completion hook. Callers are responsible for restoring the previous
    /// value when profiling ends.
    ...
    /// Called from `commit_swap_locked` just before commit so every buffer
    /// automatically carries the hook. The buffer label is whatever the
    /// encoder set via `set_label`; profilers should look at that label to
    /// identify the kernel variant.
    ...
    pub fn command_encoder_with_buffer(
        &self,
    ) -> Result<(CommandsGuard<'_>, CommandBuffer), MetalKernelError> {
        let guard = self.command_encoder()?;
        let cb = guard.guard.current.clone();
        Ok((guard, cb))
    }
    ...
    fn commit_swap_locked(
        ...
        match state.current.status() {
            MTLCommandBufferStatus::NotEnqueued | MTLCommandBufferStatus::Enqueued => {
                // Install the profiler hook (if any) *before* commit so Metal
                // can retain the block before the buffer hits the GPU.
                // A no-op when no hook is installed.
