...
pub fn quantized_matmul_mv_mc_columns(dtype: GgmlDType) -> Option<usize> {
    match dtype {
        ...
        // Q2_0 small-m verify goes to the dedicated weight-bound smallm GEMM
        // (in the mm path), not the compute-bound mc — so no mc route here.
        _ => None,
    }
}
...
pub fn call_quantized_matmul_mm_t(
    ...
    // Small-m weight-bound Q2_0 GEMM (DSpark verify, 2<=m<=8): one simdgroup per
    // 8 weight rows, weight read once. The 64x32 tile mm below wastes ~4x compute
    // on padding at this m; the mc mv is compute-bound on the ternary dot.
    // Requires a contiguous f32 activation [m, k] (the fwd path casts bf16->f32
    // and passes contiguous src1) and a single batch.
        ...
        && nb11 == ne00 * 4
    {
        ...
        encoder.dispatch_thread_groups(
            MTLSize { width: divide(ne01 as usize, 8), height: 1, depth: 1 },
