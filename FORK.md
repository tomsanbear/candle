...
| Batched simdgroup-per-row layernorm/rmsnorm + rows>=32 heuristic | metal-kernels | phase-3 | **proven: m3 bench** — layer_norm 1024×512: 2.5–2.7× (43.9→17.6 µs f32); rms 1024×512: 1.5–1.6×; 1024×1024: 1.3–2.3×; rows=1 flat (heuristic keeps stock kernels) | ported from wolfrpsiw metal_fused.rs; one measured loss: rms f32 at 1024 cols (−11%) → gated to cols ≤ 512 for that dtype only; thresholds are two-point m3 samples, refine at graduation. Upstream posture: #3150 territory — take the receipts to an issue before PRing |
